...
from models.enhancement import LabelSmoothingCrossEntropy, LabelSmoothingTarget
from models.poison_data import MyDataset_cifar, MyDataset_mnist, MyDataset_cifar_DBA, MyDataset_mnist_DBA
from models.helper import gaussian_noise_ls
...
class LocalUpdate(object):
    def __init__(self, args, dataset=None, idxs=None, epoch=None, data_size=600, epoch_size=30):
        self.args = args
        self.loss_func = nn.CrossEntropyLoss()
        self.loss_func_poe = LabelSmoothingCrossEntropy()
    ...
    def train(self, net):
        ...
        for iter in range(self.args.local_ep):
            ...
            for key, data in net.state_dict().items():
                # add dp noise
                sensitivity = 4 * 8 / (len(self.dataset) * 10)
                data += gaussian_noise_ls(data.shape, sensitivity, 1, device=self.args.device)
        ...
        return sum(epoch_loss)/len(epoch_loss), epochs_local_update_list, update
    ...
    def train_po(self, net):
        ...
        optimizer = torch.optim.SGD(net.parameters(), lr=self.args.lr_ba,
                                    momentum=0.9, weight_decay=5e-4)
        if self.args.dataset == 'cifar10' or self.args.dataset == 'gtsrb':
            if self.args.attack_methods == 'CBA' or self.args.attack_methods == 'MP':
        ...
        for iter in range(self.args.local_ep_ba):
            ...
            for batch_idx, (images, labels) in enumerate(ldr_train):
                ...
                if self.args.use_en and self.epoch <= self.args.epoch_en:
                    class_loss = self.loss_func_poe(log_probs, labels, smoothing=self.args.epsilon)
                else:
                    class_loss = self.loss_func(log_probs, labels)
                if self.args.attack_methods == 'MP':
